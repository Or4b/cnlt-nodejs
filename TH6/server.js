...
const server = http.createServer((req, res) => {
  ...
  if (route === "/" || route === "/index.html") {
    ...
    fs.readFile(
      path.join(__dirname, "views", "request.html"),
      "utf8",
      (err, template) => {
    ...
    fs.readFile(
      path.join(__dirname, "views", "streams.html"),
      "utf8",
      (err, data) => {
        res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
        res.end(data);
  ...
  // 4.2 Writable: Ghi dữ liệu form vào file
    ...
    req.on("end", () => {
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
      res.end(
        '<h2 style="color:green">Đã ghi Stream thành công vào data/log.txt!</h2>',
      );
    });
  }
  ...
  // 4.4 Duplex: Bắt request -> Echo ngược lại ngay lập tức
    ...
    req.on("end", () => {
      ...
      const textToEcho = params.get("echotext");

      const duplexStream = new EchoDuplex();
      res.writeHead(200, { "Content-Type": "text/plain; charset=utf-8" });

      // Ghi dữ liệu vào Duplex và Pipe kết quả ra Response
      duplexStream.pipe(res);
      duplexStream.write(`Server nhận được: ${textToEcho}`);
      duplexStream.end();
    });
  }
  ...
  // API: Kích hoạt sự kiện
    ...
    res.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
    res.end(
      "<h2>Đã trigger Event. Kiểm tra log Terminal!</h2><script>setTimeout(()=>window.close(), 1500)</script>",
    );
  }
  // API: Trả về JSON
  else if (route === "/json") {
  ...
  // API: Stream Hình ảnh
    ...
    if (fs.existsSync(imgPath)) {
      ...
      res.end(
        "<h2>LỖI: Chưa có ảnh!</h2><p>Hãy copy 1 tấm ảnh bất kỳ, đổi tên thành <b>test.jpg</b> và bỏ vào thư mục <b>public/images/</b></p>",
      );
    }
  }
  ...
  // LỖI 404 (Không có đường dẫn nào khớp)
    ...
    res.writeHead(404, { "Content-Type": "text/html; charset=utf-8" });
    res.end(
      '<h1 style="color:red;text-align:center;margin-top:50px;">404 - NOT FOUND</h1>',
    );
  }
});
